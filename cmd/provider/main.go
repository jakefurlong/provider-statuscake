...
import (
	...
	"github.com/jakefurlong/provider-statuscake/apis/v1alpha1"
	"github.com/jakefurlong/provider-statuscake/config"
	"github.com/jakefurlong/provider-statuscake/internal/clients"
	"github.com/jakefurlong/provider-statuscake/internal/controller"
	"github.com/jakefurlong/provider-statuscake/internal/features"
)

func main() {
	var (
		app                     = kingpin.New(filepath.Base(os.Args[0]), "Terraform based Crossplane provider for StatusCake").DefaultEnvars()
	...
	log := logging.NewLogrLogger(zl.WithName("provider-statuscake"))
	if *debug {
	...
	mgr, err := ctrl.NewManager(cfg, ctrl.Options{
		...
		LeaderElectionID: "crossplane-leader-election-provider-statuscake",
		Cache: cache.Options{
	...
	kingpin.FatalIfError(apis.AddToScheme(mgr.GetScheme()), "Cannot add StatusCake APIs to scheme")
	...
	kingpin.FatalIfError(controller.Setup(mgr, o), "Cannot setup StatusCake controllers")
